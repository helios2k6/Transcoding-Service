...
public final class MKVInfoFactory{
	 ...
	 * @return
	 */
	public static Optional<MKVInfo> tryGetMKVInfo(Path mediaFilePath, MKVInfoSource source){
		Log.info("Querying for MKVInfo for: " + mediaFilePath);
		
		Optional<String> rawQuery = source.tryGetMKVInfo(mediaFilePath);

		if(rawQuery.isSome()){
			Log.info("Successfully got MKVInfo for: " + mediaFilePath);
			return MKVInfoDeserializer.deserializeRawMKVInfo(mediaFilePath, rawQuery.getValue());
		}
	 ...
	 * @return
	 */
	public static Optional<MKVInfo> tryGetMKVInfo(Path mediaFilePath){
		return tryGetMKVInfo(mediaFilePath, ExternalProcessMKVInfoSource.getInstance());
	}
}
