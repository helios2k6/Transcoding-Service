...
public final class NeroAacArgumentBuilder implements EncoderArgumentBuilder {
	...
	private static void addProcessName(List<String> arguments){
		arguments.add(0, SystemUtilities.getNeroAacEncProcessName());
	}
	
	private static void addInputFile(List<String> arguments, EncodingJob job){
		arguments.add(InputFileArgument);
		arguments.add(job.getSourceFilePath().toAbsolutePath().toString());
	}
	
	private static void addBitRateArgument(List<String> arguments, EncodingJob job){
	...
	private static boolean shouldUseHighBitRate(EncodingJob job){
		Optional<AudioTrack> audioTrack = job.getAudioTrackOption().getAudioTrack();
	...
	private static void addFlagArguments(List<String> arguments){
		arguments.add(LowComplexityArgument);
		arguments.add(TwoPassArgument);
	}
	
	private static void addOutputArgument(List<String> arguments, Path outputFile){
